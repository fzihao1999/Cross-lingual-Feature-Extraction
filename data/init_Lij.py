...
def get_ij(pos_i, pos_j):
    ...
    i = 0
    j = 0
    len1 = min(600, len(pos_i))
    len2 = min(600, len(pos_j))
    #print(len1, len2)
    n = 0
    L = 0
    dis = []
...
def init_Lij(load_file, language):
    ...
    if load_file == "0":
        ...
        with open("./wiki_data/pos/" + filename, 'r', encoding='utf8') as file:
            word_pos = json.load(file)

    time2 = time.time()
    print(time2 - time1)

    words = []
    filename = language + "_3_10.vec"
    with open("./wiki_data/vec/" + filename, 'r', encoding='utf8') as file:
        ...
        for line in file.readlines():
            ...
            if len(word) == 0:
                ...
                words.append(word)
    sem_vec = calc_vec(word_pos, words[:200000])
    filename = language + "_sem_3_10.vec"
    save_vec('./wiki_data/sem_norm_vec/' + filename, sem_vec)
