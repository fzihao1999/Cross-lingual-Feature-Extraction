...
def main():
    ...
    src_words, x, dim = embeddings.read(srcfile, dtype=dtype)
    trg_words, z, dim = embeddings.read(trgfile, dtype=dtype)
    ...
    if args.retrieval == 'nn':  # Standard nearest neighbor
        ...
        for i in range(0, 4000, BATCH_SIZE):
            j = min(i + BATCH_SIZE, 4000)
            similarities = 2*x[i:j].dot(z.T) - knn_sim_bwd  # Equivalent to the real CSLS scores for NN
            nn = similarities.argmax(axis=1).tolist()
            for k in range(j-i):
                translation[i+k] = nn[k]

    out_file = open(args.output, 'w', encoding='utf8')
    for i in range(4000):
        if src_words[i].isspace() or trg_words[translation[i]].isspace():
            continue
        if len(src_words[i]) + len(trg_words[translation[i]]) < 2:
            continue
        out_file.write(src_words[i] + " " + trg_words[translation[i]] + "\n")
    # Compute accuracy
