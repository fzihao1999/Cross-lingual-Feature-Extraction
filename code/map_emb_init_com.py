# Our train code is based on VecMap
...
def main():
    ...
    # four new parse
    parser.add_argument('--src_semvec', help='the input source semantic embeddings')
    parser.add_argument('--trg_semvec', help='the input target semantic embeddings')
    parser.add_argument('--sem_dim', help='the dimension of semantic embeddings used in initialization')
    parser.add_argument('--add_mat', help='the combination coefficient')
    ...
    src_words_pre, x_pre, dim = embeddings.read(srcfile, threshold=200000, dtype=dtype)
    trg_words_pre, z_pre, dim = embeddings.read(trgfile, threshold=200000, dtype=dtype)
    if args.src_semvec is not None:
        src_semfile = open(args.src_semvec, encoding=args.encoding, errors='surrogateescape')
        trg_semfile = open(args.trg_semvec, encoding=args.encoding, errors='surrogateescape')
        src_words_sem, x_sem, dim = embeddings.read(src_semfile, dtype=dtype)
        trg_words_sem, z_sem, dim = embeddings.read(trg_semfile, dtype=dtype)

    # Build word to index map
    if args.src_semvec is not None:
        src_word2ind_sem = {word: i for i, word in enumerate(src_words_sem)}
        trg_word2ind_sem = {word: i for i, word in enumerate(trg_words_sem)}

    #norm
    x_no_norm = x_pre
    z_no_norm = z_pre
    x_pre = embeddings.min_max_norm(x_pre)
    z_pre = embeddings.min_max_norm(z_pre)
    x_sem = embeddings.min_max_norm(x_sem)
    z_sem = embeddings.min_max_norm(z_sem)

    # Combine two kinds of vec
    if args.src_semvec is not None:
        src_words_com, x_com = embeddings.combine(src_words_pre, src_words_sem, x_pre, x_sem, src_word2ind_sem, dim, dtype=dtype, sem_dim=int(args.sem_dim))
        trg_words_com, z_com = embeddings.combine(trg_words_pre, trg_words_sem, z_pre, z_sem, trg_word2ind_sem, dim, dtype=dtype, sem_dim=int(args.sem_dim))
    src_words, x = src_words_pre, x_no_norm
    trg_words, z = trg_words_pre, z_no_norm
    print(x.shape, z.shape)
    print(x_com.shape, z_com.shape)
    ...
    if args.cuda:
        ...
        x = xp.asarray(x)
        z = xp.asarray(z)
        x_com = xp.asarray(x_com)
        z_com = xp.asarray(z_com)
    else:
        xp = np
    ...
    embeddings.normalize(x_com, args.normalize)
    embeddings.normalize(z_com, args.normalize)

    # Build the seed dictionary
    src_indices = []
    trg_indices = []
    print(args.init_unsupervised)
    if args.init_unsupervised:
        sim_size = min(x_com.shape[0], z_com.shape[0]) if args.unsupervised_vocab <= 0 else min(x_com.shape[0], z_com.shape[0], args.unsupervised_vocab)
        '''u, s, vt = xp.linalg.svd(x_com[:sim_size], full_matrices=False)
        xsim_com = (u*s).dot(u.T)
        u, s, vt = xp.linalg.svd(z_com[:sim_size], full_matrices=False)
        zsim_com = (u*s).dot(u.T)'''
        x_com = x_com[:sim_size]
        z_com = z_com[:sim_size]
        ...
        add_k = float(args.add_mat)/10.0
        #add_k = 1.0
        '''xsim = xp.add(xp.multiply(add_k, xsim_com), xp.multiply(1-add_k, xsim_pretrain))
        zsim = xp.add(xp.multiply(add_k, zsim_com), xp.multiply(1-add_k, zsim_pretrain))'''
        del u, s, vt
        xsim.sort(axis=1)
        zsim.sort(axis=1)
        ...
        embeddings.normalize(x_com, args.normalize)
        embeddings.normalize(z_com, args.normalize)
        sim_pretrain = xsim.dot(zsim.T)
        sim_com = x_com.dot(z_com.T)
        sim = xp.add(xp.multiply(add_k, sim_com), xp.multiply(1-add_k, sim_pretrain))
            ...
            if args.direction in ('forward', 'union'):
                if args.csls_neighborhood > 0:
                    for i in range(0, trg_size, simbwd.shape[0]):
                        ...
                        '''print(zw[i:j].shape, zw[i:j].dtype)
                        print(zw[i:j])
                        print((xw[:src_size].T).shape, (xw[:src_size].T).dtype)
                        print(xw[:src_size].T)
                        print(simbwd[:j-i].shape, simbwd[:j-i].dtype)
                        print(simbwd[:j-i])'''
